...
plot(vx,vl,type="l",main='Jan profile likelihood for gamma rate')
```
...
vx=seq(.1,3,length=50)
vl=-Vectorize(prof_log_lik)(vx)
plot(vx,vl,type="l",main='Jul profile likelihood for gamma rate')
```

For alpha, we can use the same method to get the profile likelihood.
...
vx=seq(.1,3,length=50)
vl=-Vectorize(prof_log_lik)(vx)
plot(vx,vl,type="l",main='Jan profile likelihood for gamma shape')
```
...
vl=-Vectorize(prof_log_lik)(vx)
plot(vx,vl,type="l",main='Jul profile likelihood for gamma shape')
```
